...
import json
import os
from Experiments import singleshot_FL
...
if __name__ == '__main__':
    ...
    training_args.add_argument('--model', type=str, default='resnet20', choices=['fc','conv',
    ...
    training_args.add_argument('--optimizer', type=str, default='sgd', choices=['sgd', 'LBFGS', 'momentum','adam','rms'],
    ...
    training_args.add_argument('--post-epochs', type=int, default=1,
    ...
    pruning_args.add_argument('--pruner', type=str, default='mag',
                        choices=['rand','mag','snip','grasp','synflow','dispfl'],
                        help='prune strategy (default: rand)')
    pruning_args.add_argument('--prune-itv', type=int, default=1,
                        help='the interval of pruning (default: 1.0)')
    pruning_args.add_argument('--compression', type=float, default=0.6,
                        help='quotient of prunable non-zero prunable parameters before and after pruning '
                             '(default: 1.0 -> no spar)')
    ...
    pruning_args.add_argument('--mask-scope', type=str, default='local', choices=['global', 'local'],
    ...
    pruning_args.add_argument('--prune-batch-size', type=int, default=64,
    ...
    pruning_args.add_argument('--aggre_method', type=str, default='fedprox',
                        help='the aggregation method of FL: fedavg, fedprox, scaffold')
    pruning_args.add_argument('--data_split_mode', type=str, default='non-iid',
                        help='iid, non-iid, dir')
    pruning_args.add_argument('--user-num', type=int, default=50,  # 50
                        help='the number of clients')
    pruning_args.add_argument('--global-epoch', type=int, default=850,
                        help='the number of global epochs')
    pruning_args.add_argument('--distanced', type=int, default=1,
                        help='whether enable the distance calculation')
    pruning_args.add_argument('--client_drop_rate', type=float, default=0.6,  # 0.6
                        help='whether enable the distance calculation')
    pruning_args.add_argument('--dir_alpha', type=float, default=0.,
                        help='the hyperparameter of dirichlet distribution')
    pruning_args.add_argument('--neta_g', type=float, default=1,
                        help='the global learning rate of SCAFFOLD')
